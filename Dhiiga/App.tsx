...
const ProductPage: React.FC<{ products: Product[]; onToggleSave: (id: string) => void }> = ({ products, onToggleSave }) => {
  ...
  useEffect(() => {
    ...
    // Prefer product from already-loaded list
    ...
    // Otherwise fetch single listing
...
const App: React.FC = () => {
  ...
  const resetPassword = async (email: string) => {
    setAuthError(null);
    if (!email) {
      setAuthError('Please provide an email to reset password.');
      return;
    }
    const { data, error } = await supabase.auth.resetPasswordForEmail(email);
    if (error) setAuthError(error.message);
    else alert('If that email exists, a password reset link was sent.');
  };
